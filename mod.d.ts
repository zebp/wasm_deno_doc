...
export type ReexportKindAll = "all";

export interface ReexportKindNamespace {
  namespace: string;
}

export interface ReexportKindNamed {
  named: [string, string | null];
}

export type ReexportKind =
  | ReexportKindAll
...
export function defaultEsConfig(): EsConfig;

export class Parser {
  constructor(fileLoader: FileLoader, privateItems: boolean);
   ...
   */
  parseModule(fileName: string, syntax: Syntax, sourceCode: string): ModuleDoc;
  /**
   * Fetches `fileName` and parses it.
   */
  parse(fileName: string): Promise<DocNode[]>;
  /**
   * Parses a module and returns a list of exported items (no reexports).
   */
  parseSource(fileName: string, syntax: Syntax, sourceCode: string): DocNode[];
  /**
   * Fetches file_name, parses it, and resolves its reexports.
   */
  parseWithReexports(fileName: string): Promise<DocNode[]>;
}
